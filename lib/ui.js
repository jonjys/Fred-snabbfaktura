...
const api = (path, opt={}) => fetch(path, {credentials:'include', headers:{'content-type':'application/json', ...(opt.headers||{})}, ...opt});
function esc(s){
...
$('list').onclick = async (e) => {
  ...
  if (btn.dataset.act === 'del') {
    ...
    await api('invoices/'+id, {method:'PUT', body: JSON.stringify({status:'paid'})});
  }
  load();
};
api('auth/me').then(r => r.ok ? r.json() : null).then(d => {
