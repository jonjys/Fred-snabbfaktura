module.exports = function renderApp({ email }) {
  const who = email ? String(email).replace(/[<>&"]/g, "") : "Fred-anvandare";
  return `<!DOCTYPE html>
<html lang="sv">
<head>
<meta charset="UTF-8"/>
<meta name="viewport" content="width=device-width,initial-scale=1"/>
<title>Fred Invoice</title>
<style>
:root{--bg:#0E0E12;--card:#15151A;--line:#1E1E24;--line2:#2A2A34;--text:#F4F4F5;--muted:#8A8A94;--dim:#5A5A60;--accent:#7A5CFA;--ok:#34d399;--bad:#f87171}
...
.badge.paid{color:#052e1a;background:#34d399}.badge.sent{color:#3b2a00;background:#fbbf24}.badge.draft{color:#c4b5fd;background:#2a2150}
...
<header>
  <div>
    <h1>Fred Invoice</h1>
    <div class="sub">Fakturera pa 30 sekunder · auto-inloggad via Fred</div>
  </div>
...
<main>
  <div class="row">
    ...
    <a class="ghost" href="https://snabbfaktura.vercel.app/" target="_blank" rel="noopener">PDF-skapare</a>
  ...
  <form id="form" class="card hidden">
    ...
    <div><label>Valuta</label>
      <select name="currency"><option>SEK</option><option>EUR</option><option>USD</option><option>NOK</option><option>DKK</option><option>GBP</option></select>
    </div>
    <div><label>Moms %</label><input name="tax_rate" type="number" min="0" max="100" step="0.1" value="25"/></div>
    <div><label>Forfallodatum</label><input name="due_date" type="date"/></div>
...
let token = '';
try { token = localStorage.getItem('fred_token') || ''; } catch (e) {}
addEventListener('message', function(e){
  if (e.data && e.data.type === 'FRED_AUTH' && e.data.token) {
    token = e.data.token;
    try { localStorage.setItem('fred_token', token); } catch (err) {}
    load();
  }
});
const api = (path, opt={}) => {
...
async function load() {
  ...
  if (r.status === 401) {
    $('list').innerHTML = '<div class="empty"><b>Inte inloggad</b>Oppna den har vyn fran Fred /core/invoice.</div>';
    return;
  }
  if (!r.ok) { $('list').innerHTML = '<div class="empty"><b>Kunde inte hamta</b>'+r.status+'</div>'; return; }
  const data = await r.json();
  const items = data.invoices || [];
  $('count').textContent = items.length + ' fakturor';
  if (!items.length) {
    $('list').innerHTML = '<div class="card empty"><b>Inga fakturor annu</b>Skapa den forsta - den sparas pa ditt Fred-konto.</div>';
    return;
  }
